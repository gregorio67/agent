...
public class CronParser {
    ...
    public static  long getNextScheduledTime(final String cronEntry, long currentTime) throws Exception {
    ...
    protected static  long doUpdateCurrentMonth(Calendar working, CronEntry month) throws Exception {
    ...
    protected static  long doUpdateCurrentDay(Calendar working, CronEntry dayOfMonth, CronEntry dayOfWeek) throws Exception {
    ...
    public static  void validate(final String cronEntry) throws Exception {
        List<String> list = tokenize(cronEntry);
        List<CronEntry> entries = buildCronEntries(list);
    ...
    static  void validate(final CronEntry entry) throws Exception {
        ...
        if (list.isEmpty() || list.get(0).intValue() < entry.start || list.get(list.size() - 1).intValue() > entry.end) {
            throw new Exception("Invalid token: " + entry);
        }
    }

    static  int getNext(final CronEntry entry, final int current) throws Exception {
        int result = 0;
    ...
    static  boolean isCurrent(final CronEntry entry, final int current) throws Exception {
    ...
    public static void main(String[] args) throws Exception {
    	String cronEntry = "0 */5 * * * ?";
    	long time = CronParser.getNextScheduledTime(cronEntry, System.currentTimeMillis());
