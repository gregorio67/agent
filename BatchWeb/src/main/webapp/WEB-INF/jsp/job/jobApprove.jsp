...
<title>Batch Schedule Approve</title>
...
$(document).ready(function(){
	...
	$(document).on("click", "#board_list > tbody > tr > td", function(event){
		...
		/** Check job name, if job name is not null, then call function **/
	...
	$("#jobDelete").on("click", function(event){
		...
		if(!($("#board_list tbody input[type='checkbox']").is(":checked"))){
			var msg = "You should choice the row for delete";
...
function fn_Search(pageIndex){
	...
	if ($("#searchApproveStatus").prop('checked')) {
		sendParam.approveStatus = 'Y';
	}
...
function fn_searchCallBack(result){
	...
	if(result.page.totalRowCount == 0){
		...
		$.each(result.dataList, function(i, val){
			...
			dataList += '<td class="list_td" id="jobExeuctePeriod">' + gfn_nvl(val.jobExeuctePeriod) + '</td>';
			dataList += '<td class="list_td" id="jobShellLoc">' + gfn_nvl(val.jobShellLoc) + '</td>';
			dataList += '<td class="list_td" id="jobShellName">' + gfn_nvl(val.jobShellName) + '</td>';
...
function fn_keySearchCallBack(result) {
	...
	if (result.data.approveStatus != 'Y') {
		$("#jobDetail .list_btn_group #btnJobStart").hide();
		$("#jobDetail #search #approveStatus").prop("checked", false);		
	}
	else {
		$("#jobDetail .list_btn_group #btnJobStart").show();
		$("#jobDetail #search #approveStatus").prop("checked", true);		
	}
	/** UseYn is not Yes, then Job Start Button hide **/
...
function fn_insert() {
	...
	if ($("#jobDetail #search #approveStatus").prop("checked", true)) {
		sendParam.approveStatus = 'Y';
	}
	else {
		sendParam.approveStatus = 'N';		
	}
	if ($("#jobDetail #search #useYn").prop("checked", true)) {
		sendParam.useYn = 'Y';
	}
	else {
		sendParam.useYn = 'N';		
...
function fn_update(){
	...
	sendParam.jobShellName = $("#jobDetail #msearcherge #jobShellName").val();
			...
			<table class="search_table">
				...
				<tbody>
					<tr class="search_row">
						...
						<td>
							<input class="search_input" type="checkbox" id="searchApproveStatus" maxlength="10"/>
						</td>
					</tr>
				</tbody>
			</table>
					...
					<table class="list_table">
						...
						<tbody>
								...
								<tr class="detail_view">
									...
									<td class="list_td align-left">
										<input type="checkbox" name="approveStatus" id="approveStatus" class="floatL" maxlength="10">
									</td>
...
<!-- --------------------------------------------------------------------------------- -->
<!--  When click insert, show this division                                            -->
<!-- -----------------------------------merge----------------------------------------- -->			
